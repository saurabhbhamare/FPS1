...
public class ScreecherController : EnemyController
{
    ...
    public ScreecherController(EnemySO enemySO, Transform playerTransform, BulletService bulletService) : base(enemySO, playerTransform)
    {
        ...
        InitializeScreecherData(); 
        this.playerTransform = playerTransform;  
    }
    private void InitializeScreecherData()
    {
    ...
    public override void Movement()
    {
        ...
        float zigzagOffset = Mathf.Sin(Time.time * /*enemySO.zigzagFrequency*/ GetEnemyModel().zigzagFrequency) * GetEnemyModel().zigzagAmplitude;

        // Calculate the target position with zigzag offset
        Vector3 targetPosition = new Vector3(this.targetTransform.position.x + zigzagOffset, targetTransform.position.y + GetEnemyModel().hoverHeight, targetTransform.position.z);

        // Move towards the target position
        this.enemyView.transform.position = Vector3.MoveTowards(this.enemyView.transform.position, targetPosition,Time.deltaTime);
        ...
        this.enemyView.transform.rotation = Quaternion.Slerp(this.enemyView.transform.rotation, lookRotation, Time.deltaTime);
    }
